// Модульна робота №1
// Студент: Однорог Іван Євгенійович, група ПД-23

using System.Text;

class Program
{
    // Делегат для операцій над одним текстовим рядком.
    ...
    static void Main()
    {
        ...
        // Перед новою обробкою очищаємо файл результатів.
    ...
    // Метод читає файл, виконує передану операцію для кожного рядка і дописує результат.
    ...
    // Операція 1: переведення рядка у верхній регістр.
    static string ToUpperCase(string line)
    {
        return line.ToUpper();
    }

    // Операція 2: підрахунок символів у рядку.
    static string CountCharacters(string line)
    {
        return $"Рядок: \"{line}\" | Символів: {line.Length}";
    }

    // Операція 3: підрахунок слів у рядку.
    ...
    // Якщо файл з текстом відсутній або порожній, створюємо тестові рядки.
    ...
    // Створюємо порожній файл, якщо його ще немає.
...
class MessagePublisher
{
    ...
    // Метод відправляє повідомлення і викликає подію.
...
class FileLogger
{
    ...
    // Підписка логера на подію відправки повідомлення.
    public void Subscribe(MessagePublisher publisher)
    {
        publisher.MessageSent += LogMessage;
    }

    // Запис повідомлення у файл у форматі: [час] повідомлення.
